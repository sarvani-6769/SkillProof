...
const Sidebar = () => {
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200/80 min-h-[calc(100vh-4rem)] p-4 flex flex-col justify-between shrink-0 hidden lg:flex">
      <div>
        {/* User Card */}
        <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-rose-50/30 border border-slate-200/60 mb-6">
          <div className="flex items-center gap-3">
            {user?.profilePhoto ? (
              <img
                src={user.profilePhoto}
                alt={user.name}
                className="w-11 h-11 rounded-xl object-cover ring-2 ring-rose-500/20 shadow-xs"
              />
            ) : (
              <div className="w-11 h-11 rounded-xl bg-rose-600 text-white flex items-center justify-center font-bold text-sm shadow-xs">
          ...
          {isStudent && user?.profileCompletion !== undefined && (
            <div className="mt-3 pt-3 border-t border-slate-200/60">
              <div className="flex justify-between items-center text-xs font-semibold text-slate-600 mb-1">
                ...
                <span className="text-rose-600">{user.profileCompletion}%</span>
              </div>
      ...
      {isStudent && user?.username && (
        <div className="mt-6 pt-4 border-t border-slate-100">
          <NavLink
            ...
            className="flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold text-rose-600 bg-rose-50/60 hover:bg-rose-100/60 transition"
          >
            <span className="flex items-center gap-2">
              <ExternalLink className="w-3.5 h-3.5" />
              Public Portfolio View
            </span>
            <span className="text-[10px] bg-rose-200/60 text-rose-800 px-1.5 py-0.5 rounded font-bold">
              LIVE
            </span>
          </NavLink>
        </div>
